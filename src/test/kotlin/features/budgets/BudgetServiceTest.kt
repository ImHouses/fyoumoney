...
class BudgetServiceTest {
    ...
    @Test
    fun `getOrCreateBudget creates budget with all active categories`() =
        runBlocking {
    ...
    @Test
    fun `getOrCreateBudget returns existing budget on second call`() =
        runBlocking {
            categoryService.create(NewCategory("Food", TransactionType.EXPENSE, 50000))
    ...
    @Test
    fun `getOrCreateBudget excludes soft-deleted categories`() =
        runBlocking {
            categoryService.create(NewCategory("Food", TransactionType.EXPENSE, 50000))
    ...
    @Test
    fun `getOrCreateBudget copies default allocation from category`() =
        runBlocking {
            categoryService.create(NewCategory("Food", TransactionType.EXPENSE, 50000))
            val response = budgetService.getOrCreateBudget(2026, 3)
    ...
    @Test
    fun `getOrCreateBudget returns spent totals as zero when no transactions`() =
        runBlocking {
            categoryService.create(NewCategory("Food", TransactionType.EXPENSE, 50000))
            val response = budgetService.getOrCreateBudget(2026, 3)
    ...
    @Test
    fun `updateBudgetItem changes allocation`() =
        runBlocking {
            categoryService.create(NewCategory("Food", TransactionType.EXPENSE, 50000))
    ...
    @Test
    fun `updateBudgetItem changes snoozed flag`() =
        runBlocking {
            categoryService.create(NewCategory("Food", TransactionType.EXPENSE, 50000))
            val budget = budgetService.getOrCreateBudget(2026, 3)
            val itemId = budget.items[0].id
    ...
    @Test
    fun `getAllBudgets returns all budgets`() =
        runBlocking {
            categoryService.create(NewCategory("Food", TransactionType.EXPENSE, 50000))
    ...
    @Test
    fun `getAllBudgets filters by year`() =
        runBlocking {
            categoryService.create(NewCategory("Food", TransactionType.EXPENSE, 50000))
            budgetService.getOrCreateBudget(2026, 1)
    ...
    @Test
    fun `findBudgetItemForTransaction returns existing item`() =
        runBlocking {
    ...
    @Test
    fun `findBudgetItemForTransaction auto-creates budget if needed`() =
        runBlocking {
            val categoryId = categoryService.create(NewCategory("Food", TransactionType.EXPENSE, 50000))
            val item = budgetService.findBudgetItemForTransaction(categoryId, 2026, 3)
            assertNotNull(item)
