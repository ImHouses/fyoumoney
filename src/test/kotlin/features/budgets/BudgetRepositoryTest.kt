...
class BudgetRepositoryTest {
    ...
    @Test
    fun `createBudgetWithItems stores budget and items atomically`() =
        runBlocking {
            val categoryId =
            ...
            val budget =
                budgetRepository.createBudgetWithItems(
                    2026,
                    3,
                    listOf(categoryId to 50000L),
                )
    ...
    @Test
    fun `findBudgetByYearMonth returns the budget`() =
        runBlocking {
    ...
    @Test
    fun `findBudgetByYearMonth returns null when not found`() =
        runBlocking {
            assertNull(budgetRepository.findBudgetByYearMonth(2026, 3))
        }

    @Test
    fun `findBudgetItemByBudgetAndCategory returns the item`() =
        runBlocking {
            val categoryId =
                categoryRepository.create(
                    NewCategory("Food", TransactionType.EXPENSE, 50000),
    ...
    @Test
    fun `updateBudgetItem changes allocation and snoozed`() =
        runBlocking {
            val categoryId =
                categoryRepository.create(
                    NewCategory("Food", TransactionType.EXPENSE, 50000),
    ...
    @Test
    fun `findAllBudgets returns all budgets`() =
        runBlocking {
    ...
    @Test
    fun `findAllBudgets filters by year`() =
        runBlocking {
            budgetRepository.createBudgetWithItems(2026, 1, emptyList())
            budgetRepository.createBudgetWithItems(2026, 2, emptyList())
            budgetRepository.createBudgetWithItems(2025, 12, emptyList())
    ...
    @Test
    fun `findBudgetItemById returns the item`() =
        runBlocking {
            val categoryId =
                categoryRepository.create(
                    NewCategory("Food", TransactionType.EXPENSE, 50000),
